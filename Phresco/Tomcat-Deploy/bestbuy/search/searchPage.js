//Clear Searchtxt var, in case of refresh in Chrome.
$('#Searchtxt').html('');
	...
	$
			.ajax({
				...
				success : function(data) {
					...
					// Render Did you mean/
					var base = myobject.suggestQueryInfo;
					if (base.didYouMean != null && base.didYouMean != '') {
						var didYouMeanValue = base.didYouMean;
						$(".queryResults:last").prepend(
								"<p class='didYouMean'>Did you mean: <a class='didYouMean url' onClick=\"Javascript: setQuery(\'" + didYouMeanValue + "\');\">" + didYouMeanValue
										+ "</a></p></br>");
					}
					if (base.showingResultsFor != null && base.showingResultsFor != '') {
						var showingResultsForValue = base.showingResultsFor;
						$(".queryResults:last").prepend(
								"<p class='showingResultsFor'>Showing Results for: <a class='showingResultsFor url' onClick=\"Javascript: setQuery(\'" + showingResultsForValue + "\');\">" + showingResultsForValue
										+ "</a></p></br>");
					}					
...
function checkProperty(property) {
	// property = property.replace("Rocketfish&#153;","Rocketfishª");
	// alert (property);
                                ...
                                success: function (data) {
                                    data.Version = (typeof data.Version === 'undefined') ? "No Version Sent" : data.Version;
                                     data.Revision = (typeof data.Revision === 'undefined') ? "No Revision Sent" : data.Revision;
  									$(".footer").remove();
                                    $("body").append("<div class='footer'> &copy; 2013 Best Buy Co. All Rights Reserved - Version: "+data.Version + " Revision: "+data.Revision+"</div>");
							}
						}); 
