...
public class AbstractBaseScreen {
	...
	public void getImageSrc(String value) throws Exception {
		log.info("ENTERING GET IMAGE SRC VALUE OPERATION");
		try {
			String attributeImageValue = element.getAttribute("src");
			log.info("attributeImageValue:::"+attributeImageValue);
			String[] splitValue = attributeImageValue.split("/");
//			System.out.println(splitValue.length);
//			for (String splitString : splitValue) {
//				System.out.println("splitString:::"+splitString);
//			}
			int lastStringIndex = splitValue.length-1;
			String lastStringValue = splitValue[lastStringIndex];
			log.info("lastStringValue::"+lastStringValue);
			Assert.assertEquals(lastStringValue, value);
			
		} catch (Exception t) {
			throw new RuntimeException(" Exception in get image src ");
		}

	}

	public void getPlaceholderValue(String value) throws Exception {
		log.info("ENTERING GET PLACE HOLDER VALUE OPERATION");
		try {
			String placeHolderValue = element.getAttribute("placeholder");
			log.info("placeHolderValue:::"+placeHolderValue);
			Assert.assertEquals(placeHolderValue, value);
			
		} catch (Exception t) {
			throw new RuntimeException(" Exception in get place holder value  ");
		}

	}
	
	public void buttonEnabled(String xpath) throws Exception {
		log.info("ENTERING GET BUTTON ENABLED OPERATION");
		try {
			boolean buttonEnabledValue = driver.findElement(By.xpath(xpath)).isEnabled();
			log.info("buttonEnabledValue:::"+buttonEnabledValue);
			Assert.assertTrue(buttonEnabledValue);
			
		} catch (Exception t) {
			throw new RuntimeException(" Exception in buttonEnabled");
		}
